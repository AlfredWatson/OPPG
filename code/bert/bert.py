...
class Config(object):
    ...
    def __init__(self, dataset):
        ...
        self.dev_path = os.path.join(dataset, "dev")
        self.test_path = os.path.join(dataset, "data/test")
        self.class_list = ['1', '2', '3', '4', '5']  # 新数据集等级标签

        # self.class_list = [x.strip() for x in open(
        #     dataset + '/data/class.txt', encoding='UTF-8').readlines()]
        self.save_path = '/saved_dict'
        self.output_dir = 'yudong/output'
        self.save_epoch = 1
        ...
        self.with_linguistic_information_selfattention_layer = False # 是否在bertselfattention层融入语言特征
...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        # With linguistic information in embedding layer only
        elif self.with_linguistic_information_embedding_layer and \
                not self.with_linguistic_information_selfattention_layer:
            character_level = x[3]
            word_level = x[4]
            outs = self.bert(input_ids=context, attention_mask=mask, token_type_ids=token_type_ids.long(),
        ...
        # With linguistic information in self-attention layer only
        elif self.with_linguistic_information_selfattention_layer and \
                not self.with_linguistic_information_embedding_layer:
            character_level = x[3]
            word_level = x[4]
            outs = self.bert(input_ids=context, attention_mask=mask, token_type_ids=token_type_ids.long(),
        ...
        # With linguistic information in both embedding and self-attention layers
        elif self.with_linguistic_information_embedding_layer and \
                self.with_linguistic_information_selfattention_layer:
            character_level_ids = x[3]
            word_level_ids = x[4]
            ...
            outs = self.bert(input_ids=context, attention_mask=mask, token_type_ids=token_type_ids.long(),
                             character_level_ids=character_level_ids, word_level_ids=word_level_ids,
                             character_level_matrix=character_level_matrix, word_level_matrix=word_level_matrix)
